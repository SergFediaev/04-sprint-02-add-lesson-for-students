...
export const DeckItem = ({ deck }: DeckProps) => {
  ...
  const handleNotLoading = () => setIsLoading(false)

  const handleDeleteButtonClick = () => {
    handleLoading()
    dispatch(deleteDeckTC(deck.id)).finally(handleNotLoading)
  }

  const handleEditButtonClick = () => {
    handleLoading()
    dispatch(updateDeckTC({ id: deck.id, name: `${deck.name} updated` })).finally(handleNotLoading)
  }
